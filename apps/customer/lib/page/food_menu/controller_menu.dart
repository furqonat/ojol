...
class ControllerFoodMenu extends GetxController{
    ...
    try{
      ...
      if(r["data"] != null){
        ...
        for(var i = 0; i < product.length; i++){
          var status = product[i].favorites!.where((it) => it.customerId == firebase.currentUser?.uid).toList().isNotEmpty;
  ...
  cartMethod(String id_product, int quantity, int price)async{
    try{
      ...
      if(id_product.isNotEmpty && quantity != 0){
        var r = await api.cart(id_product: id_product, quantity: quantity, token: firebaseToken!);
        if(r['message'] == "OK"){
  ...
  detailProduct(BuildContext context, String imageUrl, String productName, int price, String description, bool status){
              ...
              Align(
                ...
                child: Padding(
                  ...
                  child: SizedBox(
                    ...
                    height: 60,
                    child: Card(
                      color: status ? Colors.pink.withOpacity(0.7) : Color(0xFF3978EF).withOpacity(0.7),
