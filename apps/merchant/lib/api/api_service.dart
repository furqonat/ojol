...
class ApiService {
  ...
  Future<dynamic> apiJSONGetWitFirebaseToken(
      String domainPath, String url, String token) async {
    dynamic data;
    try {
      Map<String, String> headers = {
        'content-Type': 'application/json',
      ...
      http.Response r = await http
          .get(Uri.parse("https://$domainPath.$baseUrl$url"), headers: headers);
      log("status codenya ${r.statusCode}");
  ...
  Future<dynamic> apiJSONPostWithFirebaseToken(String domainPath, String url,
    ...
    try {
      ...
      var r = await http.post(Uri.parse("https://$domainPath.$baseUrl$url"),
          headers: headers,
